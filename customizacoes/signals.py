...
from .models import Customizacao, Dependencia, Alteracao, AlteracaoAcao, Notificacao, NotificacaoTipo, Assinatura
from core.user_context import get_user

TRACK_FIELDS = [
    "tipo", "nome", "modulo", "identificador_erp", "descricao_tecnica", "conteudo", "status",
    "criado_no_erp_em", "alterado_no_erp_em", "versao", "responsavel", "responsavel_email", "is_active", "codcoligada"
]
...
def snapshot_before(sender, instance: Customizacao, **kwargs):
    if instance.pk:
        try:
            antigo = sender.get_queryset(tipo=instance.tipo).get(pk=instance.pk)
